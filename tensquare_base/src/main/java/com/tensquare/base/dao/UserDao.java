...
public interface UserDao extends JpaRepository<TCoursesEntity, String>, JpaSpecificationExecutor<TCoursesEntity> {
    ...
    @Query(nativeQuery = true, value = "SELECT a.title AS courseTitle,a.is_delete AS isDelete,a.state AS state,b.title cpTitle,b.introduce AS cpIntroduce\n" +
            "FROM t_courses a LEFT JOIN t_courses_part b ON a.id = b.courses_id WHERE a.id = ?")
    List<Map> getCourseA(CourseDTO courseDTO, Pageable pageable, Sort sort);
}
